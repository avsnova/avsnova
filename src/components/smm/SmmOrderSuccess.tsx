import { CheckCircle2, Eye, ShoppingBag, Clock, X } from "lucide-react";
...
// Elegant, premium order-success celebration (pure CSS confetti, no libs).
export default function SmmOrderSuccess({ orderId, serviceName, cost, eta, onViewOrder, onContinue }: Props) {
    ...
    <div className="fixed inset-0 z-[185] flex items-end sm:items-center justify-center" role="dialog" aria-modal="true" aria-label="Order placed">
      ...
      <div className="relative w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl border border-emerald-500/25 bg-gradient-to-b from-[#0c1a14] to-[#08040f] shadow-2xl p-6 animate-sheet-up overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <h3 className="text-lg font-bold font-space text-white">Order placed! 🎉</h3>
          <p className="text-xs text-purple-200/50 mt-1">Your campaign is now in the queue.</p>
          ...
          <div className="mt-4 rounded-2xl bg-black/30 border border-purple-500/12 p-3.5 text-left space-y-2">
            ...
            <Row label="Est. completion" value={eta} icon={<Clock className="h-3 w-3" />} />
          </div>

          <div className="mt-4 flex gap-2">
            <button onClick={onViewOrder} className="flex-1 py-3 rounded-xl bg-white/5 border border-purple-500/20 text-sm font-bold text-white hover:bg-white/10 active:scale-[0.98] transition-all cursor-pointer flex items-center justify-center gap-1.5">
